...
JAVA_MEM_OPTS=""
if [ -n "$DUBBO_MEM_OPTS" ]; then
	JAVA_MEM_OPTS=$DUBBO_MEM_OPTS
else
...
while [ $COUNT -lt 1 ]; do    
    ...
    if [ -n "$SERVER_PORT" ]; then
        ...
        echo "(echo "status";sleep 0.2;exit)|telnet $SERVER_HOST $SERVER_PORT 2> /dev/null | grep -c OK"
    	    COUNT=`(echo "status";sleep 0.2;exit)|telnet $SERVER_HOST $SERVER_PORT 2> /dev/null | grep -c OK`
        else
